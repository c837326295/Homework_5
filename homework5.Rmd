...
```{r plotting, warning = FALSE, message = FALSE, fig.width= 7}
balt_month2 %>% 
  ggplot(aes(x = yr_month, y = n, fill = season)) +
  geom_col() +
  geom_smooth(aes(x = yr_month, y = n, group = 1),
    inherit.aes = FALSE,
    color = "blue", se = FALSE, span = 0.2) +
  ...
  geom_vline(xintercept = as.Date("2015-04-01"),
             ...
             linetype = "dashed",
             size = 1) +
  annotate("text", x = as.Date("2014-04-01"),
           y = 43,
           label = "Arrest of",
           color = "lightgray") +
  annotate("text", x = as.Date("2014-04-01"),
